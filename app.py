...
# פונקציה בטוחה למשיכת מפתחות API שלא תקריס את השרת
...
with tab1:
    ...
    if rapid_api_key:
        with st.spinner("⏳ שואב יחסים מהרשת..."):
...
with tab2:
    ...
    with col_league:
        ...
        if rapid_api_key:
            with st.spinner("⏳ שואב דירוג ליגה..."):
                ...
                if standings:
                    ...
                    if home_pos:
                        ...
                        st.caption(f"ניצחונות: {home_pos.get('won')} | תיקו: {home_pos.get('draw')} | הפסדים: {home_pos.get('lost')}")
                    
                    if away_pos:
                        st.markdown(f"**✈️ {selected_game['away']}**")
                        st.metric("מקום", away_pos.get('position', '-'), f"{away_pos.get('points', 0)} נק'")
                        st.caption(f"ניצחונות: {away_pos.get('won')} | תיקו: {away_pos.get('draw')} | הפסדים: {away_pos.get('lost')}")
                else:
                    st.warning("לא ניתן להשיג דירוג ליגה כרגע")
        else:
            st.error("❌ חסר RAPID_API_KEY בהגדרות ה-Secrets של האפליקציה.")

with tab3:
    st.markdown("#### 🧠 מנוע ניתוח AI")
    st.markdown("**ספק AI:** Groq (Llama 3 - מהיר ומומלץ) 🚀")
...
st.markdown("---")
st.markdown("<div style='text-align:center; font-size:0.8rem; color:#7a8a99;'>SportIQ ULTRA v2 | Data from SofaScore + RapidAPI | עדכון כל 30 דקות</div>", unsafe_allow_html=True)
