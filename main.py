...
def get_data(sport: str, date: str = None):
    ...
    # אם נשלח תאריך, נמשוך משחקים עתידיים. אם לא - משחקי לייב.
    if sport == "football":
        if date:
            url = f"https://{host}/football-get-fixtures-by-date"
            params = {"date": date}
            res = requests.get(url, headers=headers, params=params, timeout=5)
        else:
            url = f"https://{host}/football-get-live-all"
            res = requests.get(url, headers=headers, timeout=5)
    else:
        # ב-NBA נשתמש בנקודת הקצה הקיימת שלך
